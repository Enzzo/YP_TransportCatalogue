...
namespace router::renderer {
    ...
    SphereProjector::SphereProjector(const geo::Coordinates& left_top,
        ...
        min_lon_ = left_top.lon;
        const double max_lon = right_bottom.lon;
    ...
    svg::Point SphereProjector::operator()(const geo::Coordinates& coords) const {
        return { (coords.lon - min_lon_) * zoom_coef_ + padding_,
    ...
    void MapRenderer::SetBorder(const tc::Stops& stops) {
        optional<double> min_lat, max_lat, min_lon, max_lon;
        for (const auto& stop : stops) {
            if (!stop->free) {
                const double lat = stop->coords.lat;
                const double lon = stop->coords.lon;
                min_lat = (min_lat) ? min(*min_lat, lat) : lat;
                max_lat = (max_lat) ? max(*max_lat, lat) : lat;
                min_lon = (min_lon) ? min(*min_lon, lon) : lon;
                max_lon = (max_lon) ? max(*max_lon, lon) : lon;
            }
        }
        if (!min_lat) {
            min_lat = max_lat = min_lon = max_lat = 0;
        }
        sphere_projector_ = SphereProjector({ *min_lat, *min_lon }, { *max_lat, *max_lon }, 
                                            settings_.width, settings_.height, settings_.padding);
    }

    void MapRenderer::SetTrail(const tc::Buses& buses) {
    ...
    void MapRenderer::SetStation(const tc::Stops& stops) {
        for (const auto& stop : stops) {
            if (!stop->free) {
                AddStation(*stop);
            }
    ...
    void MapRenderer::AddTrail(const tc::Bus& bus) {
        using namespace svg;
    ...
    void MapRenderer::AddTrailName(const tc::Bus& bus) {
        using namespace svg;
        const auto& point_begin = GetPoint(bus.stops.front()->coords);
        ...
        const auto& point_end = GetPoint(bus.stops.back()->coords);
    ...
    void MapRenderer::AddStation(const tc::Stop& stop) {
        using namespace svg;
        document_.Add(Circle().SetCenter(GetPoint(stop.coords))
                              .SetRadius(settings_.stop_radius)
                              .SetFillColor("white"s));
    }

    void MapRenderer::AddStationName(const tc::Stop& stop) {
        using namespace svg;
        Text text = Text().SetFillColor("black"s)
                          .SetPosition(GetPoint(stop.coords))
    ...
    svg::Polyline MapRenderer::CreateTrail(const tc::Bus& bus) const {
        using namespace svg;
        svg::Polyline line;
        const auto& stops = bus.stops;
        for (auto it = stops.begin(); it != stops.end(); ++it) {
            const auto& point = GetPoint((*it)->coords);
            line.AddPoint(point);
        }

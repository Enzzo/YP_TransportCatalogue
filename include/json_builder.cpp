...
namespace json {
    ...
    KeyContext& Builder::Key(std::string key) {
        if (UnableUseKey()) {
            throw std::logic_error("Key ñan't be applied"s);
        }
    ...
    DictContext& Builder::StartDict() {
        if (UnableUseStartDict()) {
            throw std::logic_error("StartDict ñan't be applied"s);
        }
    ...
    Builder& Builder::EndDict() {
        if (UnableUseEndDict()) {
            throw std::logic_error("EndDict ñan't be applied"s);
        }
        return AddNode(*nodes_stack_.back().release());
    }
    ...
    ArrayContext& Builder::StartArray() {
        if (UnableUseStartArray()) {
            throw std::logic_error("StartArray ñan't be applied"s);
        }
    ...
    Builder& Builder::EndArray() {
        if (UnableUseEndArray()) {
            throw std::logic_error("EndArray ñan't be applied"s);
        }
        return AddNode(*nodes_stack_.back().release());
    }
    ...
    Node Builder::Build() const {
        if (UnableUseBuild()) {
            throw std::logic_error("Builder ñan't be applied"s);
        }
        return node_;
    }
